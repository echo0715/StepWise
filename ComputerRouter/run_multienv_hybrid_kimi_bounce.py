"""
Hybrid Agent Runner with Bounce-Back: EvoCUA + Kimi K2.5

Like run_multienv_hybrid_kimi.py, but when stuck/milestone-failure triggers a
switch to the big model (Kimi K2.5), Kimi only runs for --kimi_steps steps
before control returns to EvoCUA.  Further stuck/milestone events can trigger
additional Kimi bursts.
...
Example Usage:
    python3 run_multienv_hybrid_kimi_bounce.py \
        --headless \
        --provider_name aws \
        --observation_type screenshot \
        --evocua_model EvoCUA-S2 \
        --kimi_model kimi-k2.5 \
        --result_dir ./hybrid_kimi_bounce_results \
...
def config() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run hybrid agent evaluation with EvoCUA + Kimi K2.5 (bounce-back mode)"
    )
    ...
    # Bounce-back config
    parser.add_argument("--kimi_steps", type=int, default=5,
                        help="Number of Kimi steps per burst before bouncing back to EvoCUA")
    ...
    # Logging config
    parser.add_argument("--result_dir", type=str, default="./hybrid_kimi_bounce_results")
...
file_handler = logging.FileHandler(
    os.path.join("logs", "hybrid-kimi-bounce-normal-{:}.log".format(datetime_str)), encoding="utf-8"
)
debug_handler = logging.FileHandler(
    os.path.join("logs", "hybrid-kimi-bounce-debug-{:}.log".format(datetime_str)), encoding="utf-8"
)
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                model_name = f"HybridBounce-{args.evocua_model}-{args.kimi_model}-k{args.kimi_steps}"
                ...
                try:
                    lib_run_single.run_single_example_hybrid_kimi_bounce(
                        ...
                        kimi_steps_per_burst=args.kimi_steps,
                    )
                except Exception as e:
                    import traceback
    ...
    with Manager() as manager:
        ...
        for i in range(num_envs):
            p = Process(
                ...
                name=f"HybridKimiBounceEnvProcess-{i+1}"
            )
            p.daemon = True
            p.start()
            processes.append(p)
                ...
                for idx, p in enumerate(processes):
                    if not p.is_alive():
                        ...
                        new_p = Process(
                            target=run_env_tasks,
                            args=(task_queue, args, shared_scores),
                            name=f"HybridKimiBounceEnvProcess-Restart-{idx+1}"
                        )
                        new_p.daemon = True
                        new_p.start()
                        processes[idx] = new_p
...
def get_result(action_space, use_model, observation_type, result_dir, total_file_json):
                        ...
                        if "hybrid_summary.json" in os.listdir(example_path):
                            try:
                                with open(os.path.join(example_path, "hybrid_summary.json"), "r") as f:
                                    summary = json.load(f)
                                    bursts = summary.get("num_kimi_bursts", 0)
                                    if bursts > 0:
                                        switch_count += 1
                                        total_kimi_bursts += bursts
                            except:
                                pass
    ...
    if not all_result:
        ...
        print(f"Total tasks: {total_count}, Tasks with Kimi bursts: {switch_count} ({switch_count/total_count*100:.2f}%)")
        print(f"Total Kimi bursts across all tasks: {total_kimi_bursts}")
        return all_result

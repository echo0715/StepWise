"""
Hybrid Agent Runner: EvoCUA + Kimi K2.5 with BERT Stuck Detection
...
3. If stuck is detected, switches to Kimi K2.5 to continue execution
...
Example Usage:
    python3 run_multienv_hybrid_kimi.py \
        --headless \
        --provider_name aws \
        --observation_type screenshot \
        --evocua_model EvoCUA-S2 \
        --kimi_model kimi-k2.5 \
        --result_dir ./hybrid_kimi_results \
...
def config() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run hybrid agent evaluation with EvoCUA + Kimi K2.5 (stuck detection)"
    )
    ...
    # Milestone judge model (still uses Claude for judging milestones)
    ...
    # Logging config
    parser.add_argument("--result_dir", type=str, default="./hybrid_kimi_results")
...
file_handler = logging.FileHandler(
    os.path.join("logs", "hybrid-kimi-normal-{:}.log".format(datetime_str)), encoding="utf-8"
)
debug_handler = logging.FileHandler(
    os.path.join("logs", "hybrid-kimi-debug-{:}.log".format(datetime_str)), encoding="utf-8"
)
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                model_name = f"Hybrid-{args.evocua_model}-{args.kimi_model}"
    ...
    with Manager() as manager:
        ...
        for i in range(num_envs):
            p = Process(
                ...
                name=f"HybridKimiEnvProcess-{i+1}"
            )
            p.daemon = True
            p.start()
            processes.append(p)
                ...
                for idx, p in enumerate(processes):
                    if not p.is_alive():
                        ...
                        new_p = Process(
                            target=run_env_tasks,
                            args=(task_queue, args, shared_scores),
                            name=f"HybridKimiEnvProcess-Restart-{idx+1}"
                        )
                        new_p.daemon = True
                        new_p.start()
                        processes[idx] = new_p

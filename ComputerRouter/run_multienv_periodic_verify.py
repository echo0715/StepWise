...
Baseline that periodically verifies agent progress instead of using stuck/milestone detectors.
Every N steps (default 5), Claude verifies whether the small model (EvoCUA) is making
meaningful progress. If not, switches to the big model (Kimi K2.5) for the rest of the task.

No BERT stuck detector or milestone detector is used.
...
Environment Variables Required:
    ...
    export ANTHROPIC_API_KEY="xxxx"     # For Claude verifier

Example Usage:
    python3 run_multienv_periodic_verify.py \\
        --headless \\
        --provider_name aws \\
        --observation_type screenshot \\
        --evocua_model EvoCUA-S2 \\
        --kimi_model kimi-k2.5 \\
        --result_dir ./periodic_verify_results \\
...
from mm_agents.milestone_detector import MilestoneJudge
...
def config() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run periodic verification baseline with EvoCUA + Kimi K2.5"
    )
    ...
    parser.add_argument("--judge_model", type=str, default="claude-sonnet-4-5-20250929",
                        help="Model used for periodic verification (defaults to Claude)")
    parser.add_argument("--verify_every_n_steps", type=int, default=5,
                        help="Run Claude verification every N steps")
    ...
    # Logging config
    parser.add_argument("--result_dir", type=str, default="./periodic_verify_results")
...
file_handler = logging.FileHandler(
    os.path.join("logs", "periodic-verify-normal-{:}.log".format(datetime_str)), encoding="utf-8"
)
debug_handler = logging.FileHandler(
    os.path.join("logs", "periodic-verify-debug-{:}.log".format(datetime_str)), encoding="utf-8"
)
...
def run_env_tasks(task_queue: Queue, args: argparse.Namespace, shared_scores: list):
    ...
    try:
        ...
        logger.info("Initializing Claude verifier for periodic verification...")
        try:
            verifier = MilestoneJudge(
                model=args.judge_model,
                max_tokens=2048,
            )
            logger.info("Claude verifier initialized.")
        except Exception as e:
            logger.warning(f"Failed to initialize Claude verifier: {e}")
            verifier = None
        ...
        while True:
            ...
            try:
                ...
                model_name = f"PeriodicVerify-{args.evocua_model}-{args.kimi_model}-every{args.verify_every_n_steps}"
                ...
                try:
                    lib_run_single.run_single_example_periodic_verify(
                        ...
                        verify_every_n_steps=args.verify_every_n_steps,
                    )
                except Exception as e:
                    import traceback
    ...
    with Manager() as manager:
        ...
        for i in range(num_envs):
            p = Process(
                ...
                name=f"PeriodicVerifyEnvProcess-{i+1}"
            )
            p.daemon = True
            p.start()
            processes.append(p)
                ...
                for idx, p in enumerate(processes):
                    if not p.is_alive():
                        ...
                        new_p = Process(
                            target=run_env_tasks,
                            args=(task_queue, args, shared_scores),
                            name=f"PeriodicVerifyEnvProcess-Restart-{idx+1}"
                        )
                        new_p.daemon = True
                        new_p.start()
                        processes[idx] = new_p

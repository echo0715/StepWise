...
class StepInfo:
    ...
    def save_step_info(self, exp_dir, save_json=False, save_screenshot=True, save_som=False):
        ...
        if save_json:
            try:
                # Build a lightweight dict with only the useful fields for human inspection
                step_data = {
                    "step": self.step,
                    "action": self.action,
                    "reward": self.reward,
                    ...
                    "goal": self.obs.get("goal", None) if isinstance(self.obs, dict) else None,
                }
                if self.agent_info is not None:
                    info = self.agent_info
                    step_data["think"] = getattr(info, "think", None)
                    chat = getattr(info, "chat_messages", None)
                    if chat is not None:
                        msgs = chat.messages if hasattr(chat, "messages") else chat
                        step_data["chat_messages"] = [dict(m) for m in msgs]
                    extra = getattr(info, "extra_info", None)
                    if extra:
                        step_data["extra_info"] = extra
                    stats_obj = getattr(info, "stats", None)
                    if stats_obj:
                        step_data["stats"] = stats_obj
                with open(exp_dir / f"step_{self.step}.json", "w") as f:
                    json.dump(step_data, f, indent=2, cls=DataclassJSONEncoder)
            except Exception as e:
                logger.warning(f"Failed to save step {self.step} as JSON: {e}")
...
class ExpArgs:
    ...
    def run(self):
        ...
        # Ensure experiment directory exists (needed for relaunch path where
        # find_incomplete creates new ExpArgs without calling prepare())
        ...
        try:
            ...
            while not step_info.is_done:  # set a limit
                ...
                step_info.save_step_info(
                    self.exp_dir, save_json=self.save_json, save_screenshot=self.save_screenshot, save_som=self.save_som
                )
...
def _aggregate_episode_stats(episode_info: list[StepInfo]):
    ...
    for key, val_list in stats.items():
        # Skip non-numeric values (strings, dicts, etc.) that can't be aggregated
        numeric = [v for v in val_list if isinstance(v, (int, float, np.number))]
        if not numeric:
            aggregated_stats[f"last_{key}"] = val_list[-1] if val_list else None
            continue
        aggregated_stats[f"cum_{key}"] = np.nansum(numeric)
        aggregated_stats[f"max_{key}"] = np.nanmax(numeric)
    ...
    for key, val in aggregated_stats.items():
        ...
        try:
            ...
            pass  # non-numeric values (str, dict, etc.)
...
class DataclassJSONEncoder(json.JSONEncoder):
    def default(self, obj):
        ...
        if hasattr(obj, "messages"):
            # Handle Discussion and similar message container objects
            return obj.messages
